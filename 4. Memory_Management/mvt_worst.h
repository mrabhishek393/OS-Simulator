// Q. Mvt memory management simulation worst fit
...
struct partition_mvt_worst {
    int start;
    int end;
    int process_id;
};

void print_menu_mvt_worst()
{
    cout << "\nMain Menu:\t1. Insert Process\t2. Delete Process\t3. Quit\n\n";
}

void print_table_mvt_worst(vector<partition_mvt_worst> partitions, int total_memory)
{
    cout << "\nMemory Table\n";
...
void mvt_worst()
{
    cout << "\nRunning MVT Worst Fit\n\n";

    int total_memory;
    cout << "Enter size of total memory: ";
    cin >> total_memory;

    vector<partition_mvt_worst> partitions;
    ...
    while(true)
    {
        ...
        if(choice == 1)
        {
            ...
            partition_mvt_worst new_partition;
        ...
        print_table_mvt_worst(partitions, total_memory);
    }
