...
#include<iostream>
using namespace std;
void calc_wt_tat(int p_id[],int at[],int bt[],int wt[],int tat[],int ft[],size_t size)
{
    int remaining_time[size]; //to store remaining burst time
    for(int i=0;i<size;i++)
    {
        remaining_time[i]=bt[i]; //initialization of remaining_time
    }
...
void avg_time(size_t t,int p_id[],int at[],int bt[])
{
    ...
    calc_wt_tat(p_id,at,bt,wt,tat,finish_time,t); //calling calc_wt_tat function
    //printing the output
...
void initialization()
{
    ...
    avg_time(t,p_id,at,bt); //calling avg_time function for calculation
    return;
}
int main()
{
