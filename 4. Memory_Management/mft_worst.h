...
void print_menu_mft_worst()
{
    cout << "\nMain Menu:\t1. Insert\t2. Delete\t3. Stop\n\n";
}

void print_partitions_mft_worst(vector<int> assigned_process, vector<int> partitions_size_alloc, vector<int> partitions, int memory_block_count)
{
    ...
    while(true)
    {
        ...
        if(choice == 1)
        {
            ...
            if(process_allocated)
            {
                ...
                print_partitions_mft_worst(assigned_process, partitions_size_alloc, partitions, memory_block_count);
                continue;
            }
            ...
            // Find the largest partition that can fit the ith process
            int max_partition = -1;
            for(int i = 0; i < partition_count; i++)
            {
                if(partition_used[i])
                ...
                if(partitions[i] >= process_size && (max_partition == -1 || partitions[i] > partitions[max_partition]))
                {
                    max_partition = i;
                }
            }
            ...
            if(max_partition != -1)
            {
                cout << "\nProcess " << process << " is at partition " << max_partition + 1 << "\n";
                partition_used[max_partition] = true;
                partitions_size_alloc[max_partition] = process_size;
                assigned_process[max_partition] = process;
                used_memory += partitions_size_alloc[max_partition];
                print_partitions_mft_worst(assigned_process, partitions_size_alloc, partitions, memory_block_count);
                continue;
            }

            if(max_partition == -1) // There was no valid partition
            {
