...
void print_menu_mft_first()
{
    cout << "\nMain Menu:\t1. Insert\t2. Delete\t3. Stop\n\n";
}

void print_partitions_mft_first(vector<int> assigned_process, vector<int> partitions_size_alloc, vector<int> partitions, int memory_block_count)
{
    ...
    while(true)
    {
        ...
        if(choice == 1)
        {
            ...
            if(process_allocated)
            {
                ...
                print_partitions_mft_first(assigned_process, partitions_size_alloc, partitions, memory_block_count);
                continue;
            }
            ...
            // Find the first partition that can fit the ith process
            ...
            {
                cout << "\nProcess " << process << " is at partition " << first_partition + 1 << "\n";
                partition_used[first_partition] = true;
                partitions_size_alloc[first_partition] = process_size;
                assigned_process[first_partition] = process;
                used_memory += partitions_size_alloc[first_partition];
                print_partitions_mft_first(assigned_process, partitions_size_alloc, partitions, memory_block_count);
                continue;
            }

            if(first_partition == -1) // There was no valid partition
            {
