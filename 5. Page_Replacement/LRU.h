 #include<bits/stdc++.h>
int preset_or_not_lru(int table_f[], int nframe, int page_is_here) //function to check if page is present or not
{
...
void printing_lru(int table_f[], int nframe)  //printing_lru the table at each step
{
	for(int i=0; i<nframe; i++)
	{
		if(table_f[i] == -1)
...
int find_position_here_lru(int table_f[], int nframe, int pages[], int curr, int np) //finds a suitable position
{
	for(int i=0; i<nframe; i++)
		if(table_f[i] == -1)
			return i;
...
void lru()
{
    ...
    for(i=0;i<n;i++)
    {
        ...
        if(!preset_or_not_lru(table_frame,nf,pages[i]))
        {
             int position_noted = find_position_here_lru(table_frame,nf,pages,i,n);
             table_frame[position_noted]=pages[i];
             printing_lru(table_frame,nf);
             printf(" page fault!\n");
