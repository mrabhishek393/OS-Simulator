...
struct partition_mvt_best {
    int start;
    int end;
    int process_id;
};

void print_menu_mvt_best()
{
    cout << "\nMain Menu:\t1. Insert Process\t2. Delete Process\t3. Quit\n\n";
}

void print_table_mvt_best(vector<partition_mvt_best> partitions, int total_memory)
{
    cout << "\nMemory Table\n";
...
void mvt_best()
{
    cout << "\nRunning MVT Best Fit\n\n";
    
    int total_memory;
    cout << "Enter size of total memory: ";
    cin >> total_memory;

    vector<partition_mvt_best> partitions;
    ...
    while(true)
    {
        ...
        if(choice == 1)
        {
            ...
            partition_mvt_best new_partition;
        ...
        print_table_mvt_best(partitions, total_memory);
    }
